defmodule LiveViewAppWeb.PresenceLive do
  use LiveViewAppWeb, :live_view

  alias LiveViewAppWeb.Presence
  ...
  def mount(_params, _session, socket) do
    ...
    if connected?(socket) do
      Phoenix.PubSub.subscribe(LiveViewApp.PubSub, @topic)
      ...
      <div class="users">
        ...
        <ul>
          <li :for={{_user_id, meta} <- @presences}>
            <span class="status">
              <%= if meta.is_playing, do: "Listening", else: "Idle" %>
            </span>
  ...
  def add_presences(socket, joins) do
    presences = Map.merge(socket.assigns.presences, presence_map(joins))
